...
RESPONSE: The most important part of the code in my opinion is the naming convention. The names need to be concise yet descriptive enough to understand what you are doing or what you mean when you call that variable name. If a name is just 'a' the next person to look at the code has no idea what one meant when they originally put that. I think making clear names even if they are long will pay back dividens in the long run. I appreciate that and also clean spacing. I cant stand when everything is jumbled up so give it a couple lines between functions/classes/etc.
...
RESPONSE: A way a container can help run a computer is by packing up everything you use to run specific apps on the computer. Instead of downloading heaps upon heaps of information it could be useful to just have a container that is needed for python. One could have a specific coding container and then maybe one for personal work, and so on. The containers help keep everything in a smaller storage format and then allow it to be open so apps can run on top of it. This is the beauty of containers.
